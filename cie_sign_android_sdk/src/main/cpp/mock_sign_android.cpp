                                   ...
                                   const AppearanceOptions& appearance)
{
    ...
    req.pdf.use_auto_signature = appearance.useAutoSignature ? 1 : 0;
    req.pdf.signer_name_override = appearance.signerNameOverride.empty() ? nullptr : appearance.signerNameOverride.c_str();
    __android_log_print(ANDROID_LOG_DEBUG, kLogTag, "PDF field ids len=%zu useAutoSig=%d",
                        fieldPointers.size(), appearance.useAutoSignature ? 1 : 0);
    result.output_len = 0;
    ...
    jstring signerNameOverride)
{
    ...
    appearance.useAutoSignature = useAutoSignature == JNI_TRUE;
    appearance.signerNameOverride = to_std_string(env, signerNameOverride);
