...
public:
    NativeRequestGuard(JNIEnv* env,
        ...
        use_auto_signature_ = useAutoSig == JNI_TRUE;
        signer_name_override_ = readString(signerNameOvr);
    }
    ...
    bool use_auto_signature() const { return use_auto_signature_; }
    const std::string& signer_name_override() const { return signer_name_override_; }
...
private:
    ...
    int image_width_ = 0;
    int image_height_ = 0;
    bool use_auto_signature_ = false;
    std::string signer_name_override_;
};
...
Java_it_ipzs_ciesign_sdk_NativeBridge_signPdfWithNfc(
    ...
    jint imageWidth,
    jint imageHeight,
    jboolean useAutoSignature,
    jstring signerNameOverride,
    jobject isoDep,
    jbyteArray atrBytes,
    jstring outputPath) {
    ...
    NativeRequestGuard request(env, pdfBytes, pin, reason, location, name, fieldIds, signatureImage, imageWidth, imageHeight, useAutoSignature, signerNameOverride, atrBytes);
    ...
    signRequest.pdf.use_auto_signature = request.use_auto_signature() ? 1 : 0;
    signRequest.pdf.signer_name_override = request.signer_name_override().empty() ? nullptr : request.signer_name_override().c_str();
