...
if [[ -d "$SRC_PREFIX/include/curl" ]]; then
  rsync -a --delete "$SRC_PREFIX/include/curl/" "$DEST/include/curl/"
else
  echo "Warning: curl headers missing" >&2
fi

cp "$SRC_PREFIX/lib/libcurl.a" "$DEST/lib/"
